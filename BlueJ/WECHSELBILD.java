...
extends Knoten
{
    ...
    private boolean schildAktiv = false;
     ...
     */
    public WECHSELBILD( int x , int y , String verzeichnis )
    {
        System.out.println(verzeichnis);
        File folder = new File("src\\Animationen\\"+verzeichnis);
        File[] bilder = folder.listFiles();
        
        String[] dateien = new String[bilder.length];
        for (int i=0; i<bilder.length; i++){
            dateien[i] = bilder[i].getName();
        ...
        this.bilder = new Bild[this.anz_Bilder+1];//animation + schild

        for ( int i=0 ; i<this.anz_Bilder ; i++ )
        {
            this.bilder[i] = new Bild("src\\Animationen\\"+verzeichnis+"\\"+dateien[i] );
            this.bilder[i].sichtbarSetzen( false );
            this.add( this.bilder[i] );
        }
        
        this.bilder[this.bilder.length-1] = new Bild("src\\schild.png");//schild
        this.bilder[this.bilder.length-1].sichtbarSetzen(false);
        this.add( this.bilder[this.bilder.length-1] );
    ...
    public void setSchield(boolean aktiv){
        this.schildAktiv = aktiv;
        this.bilder[this.bilder.length-1].sichtbarSetzen(aktiv);
    }
    /**
    ...
    {
        for (int i = 0; i < bilder.length; i++){
            bilder[i].drehenAbsolut(neuerDrehwinkel);
        }
